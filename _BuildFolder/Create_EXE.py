import PyInstaller.__main__, os, time
from cryptography.fernet import Fernet
from shutil import copy, rmtree, move
class CreateEXE:
    def __init__(self, files: list) -> None:
        self.files = files
        self.icon = os.path.join(os.getcwd(), "icons\\PyPass.ico")

    def build_program(self, name: str) -> None:
        PyInstaller.__main__.run([f"{name}.py", "--onedir", "--windowed", f"-i={self.icon}"])
            
    def run(self) -> None:
        for name in self.files:
            self.build_program(name)

# Run to build the exe
if __name__ == "__main__":
    import cProfile
    import pstats
    import logging
    
    logging.basicConfig(filename='build.log', encoding='utf-8', level=logging.DEBUG)
    
    log = logging.getLogger("Build EXE")
    
    with cProfile as prof:
        builder = CreateEXE(["main"])
        start_time = time.perf_counter() # Track execution time
        builder.run()
        copy(builder.icon, "dist/PyPass")   
        rmtree(os.path.join(os.getcwd(), "build"))
        if os.path.exists(os.path.join(os.getcwd(), "Staging", "dist")) or os.path.exists(os.path.join(os.getcwd(), "Staging", "PyPass")):
            rmtree(os.path.join(os.getcwd(), "Staging"))
        if os.path.exists(os.path.join(os.getcwd(), "Staging", "PyPass-installer.exe")):
            os.remove(os.path.join(os.getcwd(), "Staging", "PyPass-Installer.exe"))
        src = os.path.join(os.getcwd(), "dist")
        dst = os.path.join(os.getcwd(), "Staging")
        move(src, dst)
        os.remove(os.path.join(os.getcwd(), "PyPass.spec"))
        log.info(f"Done, build took {time.perf_counter() - start_time} seconds")
        log.info("Generating new master key...")
        log.info(f"Done, new master key: {Fernet.generate_key().decode()}")
    stats = pstats.Stats(prof)
    stats.sort_stats(pstats.SortKey.TIME)
    stats.dump_stats("needs_profiling.prof")
    input("Don't forget to copy this! Press any key to exit...")
    
